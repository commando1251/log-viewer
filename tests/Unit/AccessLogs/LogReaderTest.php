...
it('can read access logs with the default LogReader', function () {
    $file = new \Commando1251\LogViewer\LogFile(__DIR__.'/Fixtures/access_dummy.log');
    $file->logs()->scan();
    ...
    /** @var \Commando1251\LogViewer\Logs\HttpAccessLog $firstLog */
    $firstLog = $logs[0];
    ...
    expect($firstLog->index)->toBe(0)
        ->and($firstLog)->toBeInstanceOf(\Commando1251\LogViewer\Logs\HttpAccessLog::class)
...
it('can read nginx error logs with the default LogReader', function () {
    $file = new Commando1251\LogViewer\LogFile(__DIR__.'/Fixtures/errors_nginx_dummy.log');
    $file->logs()->scan();
    ...
    /** @var \Commando1251\LogViewer\Logs\HttpNginxErrorLog $firstLog */
    $firstLog = $logs[0];
    ...
    expect($firstLog->index)->toBe(0)
        ->and($firstLog)->toBeInstanceOf(\Commando1251\LogViewer\Logs\HttpNginxErrorLog::class)
...
it('can read apache error logs with the default LogReader', function () {
    $file = new Commando1251\LogViewer\LogFile(__DIR__.'/Fixtures/errors_dummy.log');
    $file->logs()->scan();
    ...
    /** @var \Commando1251\LogViewer\Logs\HttpApacheErrorLog $firstLog */
    $firstLog = $logs[0];
    ...
    expect($firstLog->index)->toBe(0)
        ->and($firstLog)->toBeInstanceOf(\Commando1251\LogViewer\Logs\HttpApacheErrorLog::class)
...
it('can get access log level counts', function () {
    ...
    expect($levels['500']->count)->toBe(3)
        ->and($levels['500']->level)->toBeInstanceOf(\Commando1251\LogViewer\LogLevels\HttpStatusCodeLevel::class)
